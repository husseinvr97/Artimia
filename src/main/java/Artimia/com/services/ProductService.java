...
public class ProductService {
    ...
    public ProductGet createProduct(ProductCreate productCreate, MultipartFile image) throws IOException {
        ...
        product.setQuantity(productCreate.quantity());
    ...
    public ProductGet getProductById(Long id) {
        return productsRepository.findById(id).map(ProductMapper::convertToProductGet)
                .orElseThrow(() -> new ResourceNotFoundException("Product not found"));
    }

    public ProductGet getProductByName(String productName) {
        return productsRepository.findByProductName(productName).map(ProductMapper::convertToProductGet)
                .orElseThrow(() -> new ResourceNotFoundException("Product not found"));
    }

    public Page<ProductGet> searchProducts(Style style, BigDecimal minPrice, BigDecimal maxPrice, Pageable pageable) {
        return productsRepository
                .searchProducts(style, minPrice != null ? minPrice : BigDecimal.ZERO,
                        maxPrice != null ? maxPrice : BigDecimal.valueOf(1000000), pageable)
                .map(ProductMapper::convertToProductGet);
    }

    public List<ProductGet> getTopProducts() {
        return productsRepository.findTop10ByOrderByTimesBoughtDesc().stream().map(ProductMapper::convertToProductGet)
                .toList();
    }
